...
Event::Event( TTreeReader &myReader, configuration &cmaConfig ) :
  ...
  m_DNN(0.0){
    ...
    m_true_pileup  = new TTreeReaderArray<int>(m_ttree,"pu_NtrueInt");
    ...
    if (m_config->useLargeRJets()){
      ...
      m_ljet_SDmass = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_softDropMassCHS");
      m_ljet_tau1 = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_tau1CHS");
      m_ljet_tau2 = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_tau1CHS");
      m_ljet_tau3 = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_tau1CHS");
      ...
      m_ljet_subjetIndex0 = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_vSubjetIndex0");
      m_ljet_subjetIndex1 = new TTreeReaderArray<float>(m_ttree,"jetAK8CHS_vSubjetIndex1");
    ...
    if (m_config->useLeptons()){
      ...
      m_el_iso   = new TTreeReaderArray<float>(m_ttree,"el_Iso03");

      m_el_veto    = new TTreeReaderArray<int>(m_ttree,"el_IDVeto");
      m_el_loose   = new TTreeReaderArray<int>(m_ttree,"el_IDLoose");
      m_el_medium  = new TTreeReaderArray<int>(m_ttree,"el_IDMedium");
      m_el_tight   = new TTreeReaderArray<int>(m_ttree,"el_IDTight");
      ...
      m_mu_iso   = new TTreeReaderArray<float>(m_ttree,"mu_Iso04");
    ...
    if (!m_kinematicReco){
      if (m_config->useNeutrino()){
        // Neutrino isn't stored in the baseline ntuples, requires 'kinematicReco' to create
      ...
      if (m_config->useSignal()){
        ...
        m_jet_vlq_index    = new TTreeReaderValue<int>(m_ttree, "jet_vlq_index");
        ...
        m_jet_wprime_index    = new TTreeReaderValue<int>(m_ttree, "jet_wprime_index");
      }
    }

    m_met_met  = new TTreeReaderArray<float>(m_ttree,"met_Pt");
    m_met_phi  = new TTreeReaderArray<float>(m_ttree,"met_Phi");
    m_met_met_sf = new TTreeReaderArray<float>(m_ttree,"met_uncorPt");
    m_met_phi_sf = new TTreeReaderArray<float>(m_ttree,"met_uncorPhi");
...
void Event::initialize_ljets(){
    ...
    for (unsigned int i=0,size=**m_ljet_size; i<size; i++){
        ...
        ljet.softDropMass  = m_ljet_softDropMass->At(i);
...
void Event::buildTtbar(){
    ...
    if (m_kinematicReco){
        ...
        // no kinematic reconstruction performed, obtain from data file
        Neutrino nu; // neutrino
        nu.p4.SetPtEtaPhiM( *(*m_nu_pt), *(*m_nu_eta), *(*m_nu_phi), 0.0 );
    }
...
//    delete m_lumiblock;
    if (m_config->useLargeRJets()){
      ...
      delete m_ljet_tau1;    // jetAK8CHS_tau1CHS
      delete m_ljet_tau2;    // jetAK8CHS_tau1CHS
      delete m_ljet_tau3;    // jetAK8CHS_tau1CHS
      delete m_ljet_charge;      // jetAK8CHS_Charge
      delete m_ljet_softDropMass;  // jetAK8CHS_softDropMassCHS
