...
# plot two histograms from the same file against each other
# -> h_el_pt_ejets
# -> h_el_pt_mujets
...
for fi,file in enumerate(files):
    ...
    hist.x_label     = "Lepton pT [GeV]"  #variables[histogramName].label
    hist.y_label     = "Events"
    hist.y_ratio_label = ""
    ...
    for hi,histogram in enumerate(histograms):
        ...
        label = 'Electron' if histogram.startswith('h_el') else "Muon"

        hist.Add(h_hist,name=histogram,linecolor=betterColors[numberOfHists],
                 draw='step',label=label,ratio_partner=ratio_partner,
